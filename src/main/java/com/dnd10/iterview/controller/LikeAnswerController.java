...
public class LikeAnswerController {
  ...
  @PostMapping
  public ResponseEntity<LikeAnswerResponseDto>  createLikeAnswer(Long answerId, Principal principal) {
    return ResponseEntity.ok(likeAnswerService.create(answerId, principal));
  }

  @ApiOperation(value = "답변 좋아요 삭제", notes = "<big>특정 답변의 좋아요</big>을 삭제한다.")
  @DeleteMapping
  public ResponseEntity<LikeAnswerResponseDto>  DeleteLikeAnswer(Long answerId, Principal principal) {
    return ResponseEntity.ok(likeAnswerService.delete(answerId, principal));
  }
